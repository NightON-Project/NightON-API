userdata_table_query = """CREATE TABLE IF NOT EXISTS userdata(id_user VARCHAR(255) PRIMARY KEY, firstname_user VARCHAR(80), lastname_user VARCHAR(80), birthdate_user VARCHAR(80), email_user VARCHAR(80), telephone_user VARCHAR(80), pays VARCHAR(80), code_postal VARCHAR(80), ville VARCHAR(80), numero_rue VARCHAR(80), nom_rue VARCHAR(80), complement_adresse_1 VARCHAR(80), complement_adresse_2 VARCHAR(80), url1_piece VARCHAR(255), url2_piece VARCHAR(255));"""
login_table_query = """CREATE TABLE IF NOT EXISTS login_table(email_user VARCHAR(80) PRIMARY KEY, code VARCHAR(5));"""
tenants_table_query = """CREATE TABLE IF NOT EXISTS tenants(id_tenant VARCHAR(255) PRIMARY KEY, id_user VARCHAR(255), email_user VARCHAR(255), status_demande VARCHAR(80), date_demande VARCHAR(80), id_property VARCHAR(255), starting_date_demand VARCHAR(80), ending_date_demand VARCHAR(80), CONSTRAINT fk_id_properties_table_tenants FOREIGN KEY(id_property) REFERENCES properties_table(id_property) ON DELETE CASCADE, CONSTRAINT fk_userdata_tenants FOREIGN KEY(id_user) REFERENCES userdata(id_user) ON DELETE CASCADE);"""
owners_table_query = """CREATE TABLE IF NOT EXISTS owners(id_owner VARCHAR(255) PRIMARY KEY, id_user VARCHAR(255), email_user VARCHAR(255), status_demande VARCHAR(80), date_demande VARCHAR(80), CONSTRAINT fk_userdata_owners FOREIGN KEY(id_user) REFERENCES userdata(id_user) ON DELETE CASCADE);"""
...
CREATE TABLE IF NOT EXISTS rental_agreements(
    ...
    CONSTRAINT fk_rental_agreements_properties FOREIGN KEY(id_property) REFERENCES properties_table(id_property) ON DELETE CASCADE
);"""

sessions_infos_table_query ="""
