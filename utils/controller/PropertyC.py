...
class ClassPropertyC:
    ...
    def displayPropertyByName(nom_affichage: str):
        ...
        try:
            ...
            infos_owner = {"prenom": "___", "email": "___"}
    ...
    @staticmethod
    def displayPropertyById(id):
        """Pour afficher les détails d'un seul logement par son identifiant.
        :returns : dict formatté + id_property(utilisé en cookie)
        """
        try:
            p: ClassPropertyM = ClassPropertyDAO().findOne(key=id)
            if not p:  # when p is None
                return "AUCUN LOGEMENT A CE NOM"
