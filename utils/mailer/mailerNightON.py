...
class Mailer:
    ...
    def sendEmailCode(self, emailDestination: str) -> (int, bool):
        ...
        return code, self.sendEmail(
            emailDestination, TITTLE_CODE_VERIF, HTML_FILE_CODE_VERIF, {"code": code}
        )

    def sendEmail(
        self,
        emailDestination: str,
        mailTitle: str,
        mailContentTemplateFile: str,
        placeholders: dict = {},
    ) -> bool:
        """
        ...
        print(
            f"<NIGHTON_MAILER> : mail {mailContentTemplateFile}.html envoyé à {emailDestination} avec succès"
        )
        self.server.sendmail(
            self.data["username"], emailDestination, message.as_string()
        )
        return state
    ...
    def readMailTemplate(self, mailFile: str) -> (str, bool):
        ...
        try:
            ...
            print(
                f"<NIGHTON_MAILER> : Impossible d'ouvrir le fichier mails/{mailFile}.html"
            )
            return "", False
